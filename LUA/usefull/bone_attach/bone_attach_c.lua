...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		...
		elseif isElementStreamedIn(ped) then
			...
			if (isElement(element) and tonumber(objx) ~= nil and tonumber(objy) ~= nil and tonumber(objz) ~= nil) then
				setElementPosition(element,objx,objy,objz)
			end
			if (isElement(element) and tonumber(offrx) ~= nil and tonumber(offry) ~= nil and tonumber(offrz) ~= nil) then
