...
local function checksum_header(block)
	...
	local sum = 256
	for i = 1,148 do
		sum = sum + (string.byte(block, i) or 0)
	end
	for i = 157,500 do
		sum = sum + (string.byte(block, i) or 0)
	end
...
function untar(filePath, destPath, onComplete)
	local destPath = ""

	if not fileExists(filePath) then
		outputDebugString("TAR ERROR : File Not Found, Please check the file exists in the path specified")
		return 404
	end

	--local tar_handle = _ioOpen(filePath, "rb")
	local tar_handle = fileOpen(filePath)
	if not tar_handle then outputDebugString("Error opening file "..filename) return 403 end
	...
	while true do
		...
		repeat
			...
			block = fileRead(tar_handle, blocksize)
		until (not block or string.len(block) < 1) or checksum_header(block) > 256
		if (not block or string.len(block) < 1) then break end
		local header, err = read_header_block(block)
		if (not header) then
			outputDebugString("err:"..tostring(err))
		end
		...
		local file_data = fileRead(tar_handle, math.ceil(header.size / blocksize) * blocksize):sub(1,header.size)
		...
		if header.typeflag == "directory" then
			--lfs.mkdir( pathname )
			fileClose(fileCreate(pathname..".dir"))
			fileDelete(pathname..".dir")
		elseif header.typeflag == "file" then
			--local file_handle = _ioOpen(pathname, "wb")
			local file_handle = fileCreate(pathname)
			fileWrite(file_handle, file_data)
...
local function onExtractComplete()
	outputChatBox("extracted!")
end

--Extract the file
--[[addCommandHandler("untar", function()
	untar("tartest.tar", "/", onExtractComplete)
end)]]