...
addEventHandler("bankAlarm", root, function(bool)
	...
	if (bool) then
		alarmSounds[1] = playSound3D("files/sounds/bank_alarm.mp3", 1381.2, -1085.5, 27.4, true) -- draussen
		setSoundMaxDistance(alarmSounds[1], 75)
		
		alarmSounds[2] = playSound3D("files/sounds/bank_alarm.mp3", 858.1, 1688.8, 10535.9, true) -- oben
		setElementInterior(alarmSounds[2], 20)
		setElementDimension(alarmSounds[2], 66)
		setSoundMaxDistance(alarmSounds[2], 50)
		
		alarmSounds[3] = playSound3D("files/sounds/bank_alarm.mp3", 868.9, 1701.9, 10530.8, true) -- tresor
		setElementInterior(alarmSounds[2], 20)
		setElementDimension(alarmSounds[2], 66)
		setSoundMaxDistance(alarmSounds[2], 50)
			...
			if (tonumber(getElementData(bankPed, "angst")) > 0 and not isPedDead(bankPed)) then
				...
				if (getDistanceBetweenPoints3D(mx, my, mz, x, y, z)	<= 12) then			
					...
					if (sx and sy) then
						...
						dxDrawText("Angstlevel: "..angst.." %", sx-85, sy, sx+85, sy+25,  tocolor(0, 0, 0, 255), 1.1, 'default-bold', 'center', 'center')
...
addEventHandler("onClientPedDamage", root, function(attacker, weapon)
	...
	elseif (getElementData(source, "bankPed")) then
		if (false) then --#TODO#: faction check
			cancelEvent()
		end
