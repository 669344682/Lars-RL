...
local isOpen = false
local function CluckinBell(pedData)
	if (isOpen) then return end
	isOpen = true
	local Menues = settings.systems.shops.cluckinbell.menues
	...
	setCameraMatrix(px+1.5, py-1.5, pz+1, x+0.3, y, z, 0, 70)
	...
	buyMenu = function(nr)
		...
		local buyFix = function(nr)
			...
			closeIt()
			triggerServerEvent("cluckinBellBuy", localPlayer, currentProduct, bool)
		end
		local _price = Menues[currentProduct].price
		displayProductSelect("Cluckin’ Bell", {[1] = {title = "Hier Essen", price = _price}, [2] = {title = "Zum Mitnehmen", price = _price}}, function()end, buyFix, closeIt, "files/images/productlist/cluckin_bell.png")
	end
	
	displayProductSelect("Cluckin’ Bell", Menues, changeMenu, buyMenu, closeIt, "files/images/productlist/cluckin_bell.png")
end
addEvent("cluckinBellHello", true)
addEventHandler("cluckinBellHello", root, CluckinBell)