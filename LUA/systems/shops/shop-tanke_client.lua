...
local function Tankstellenshop()
	local items		= settings.systems.shops.tankstelle.menues
	
	local window	= dxWindow:new("Tankstellenshop", nil, nil, 500, 390, nil, nil, nil, nil, nil, true, function() triggerServerEvent("toggleGhostMode", localPlayer, localPlayer, false) end)
	local infoTxt	= dxText:new(10, 5, 480, 20, "Willkommen im Xoomer Tankstellenshop!", window, "center", 1, 'exo-10', nil, false)
	...
	local buyIt = function()
		...
		if (tonumber(count) ~= nil and tonumber(count) > 0) then
			...
			triggerServerEvent("tankeShopBuy", localPlayer, i, tonumber(count))
		end
	end
...
addEvent("tankeShopHello", true)
addEventHandler("tankeShopHello", root, Tankstellenshop)